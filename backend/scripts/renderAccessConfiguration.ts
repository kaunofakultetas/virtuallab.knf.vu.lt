...
async function writeBundle(
    ...
    for (const [absolutePath, content] of Object.entries(files).sort(([left], [right]) => left.localeCompare(right))) {
...
async function main(): Promise<void> {
    ...
    if (!inputPath) {
        throw new Error("Usage: npm run render-access -- <desired-state-input.json> [--output-dir <directory>]");
    }
    ...
    ) as AccessDesiredStateInput;
    const rendered = renderAccessConfiguration(buildAccessPlan(input));
    if (outputDirectory) {
